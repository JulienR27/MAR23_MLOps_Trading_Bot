...
def put_users(user: User, username: str = Depends(get_admin)):
   ...
   with open('users_db.csv', 'a') as f_object:
      ...
      dictwriter_object.writerow(users_dict)
...
def predict(params: Params, username: str = Depends(get_current_user)):
    '''
...
def predict_best_stocks(params: ParamsBest, username: str = Depends(get_current_user)):
   '''
